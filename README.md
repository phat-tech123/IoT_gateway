...
- `ESP32_MQTT_Test`: mã nguồn cho hai ESP32 (ESP1 và ESP2)
- `gateway/mqtt_gateway.py`: Python gateway chuyển tiếp dữ liệu giữa local MQTT broker và CoreIoT
- `description_system.txt`: ghi chú yêu cầu và topic MQTT

### Mục đích hiện tại
Gateway thu thập telemetry/status/anomaly từ ESP32, gửi lên CoreIoT và nhận lệnh điều khiển LED từ dashboard.
...
- Subscribe LED command:
  ...
  - Payload mẫu: `{ "state": "on" }` hoặc `{ "state": "off" }`
- Publish status:
  - Topic: `esp32/esp1/status`
  - Payload mẫu: `{ "status": "online", "rssi": -60 }`
- Last will: `esp32/esp1/status` = `offline` nếu mất kết nối MQTT

### ESP2
- Client ID: `ESP32_002`
- Device name: `ESP2`
- Subscribe ESP1 telemetry:
  - Topic: `esp32/esp1/telemetry`
...
- Publish status:
  ...
  - Payload mẫu: `{ "status": "online" }`

### Hành vi chung
- ESP1 gửi telemetry mỗi ~2 giây
- ESP1 / ESP2 gửi status mỗi ~10 giây
- ESP2 gửi anomaly every 5 seconds nếu đã nhận telemetry ESP1
- ESP1 và ESP2 đều xử lý command LED từ topic `esp32/espX/cmd/led`
...
Theo ảnh `Dashboard.png`, dashboard hiện có:
...
- gateway hiện hỗ trợ nested JSON như:
